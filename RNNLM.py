...
class Config(object):
  ...
  max_epochs = 100 # the number of max epoch
  early_stopping = 5
  dropout = 0.1
  lr = 0.001
  momentum = 0.0
...
class RNNLM_Model(nn.Module):
  ...
  def add_embedding(self, input_x):
    ...
    """
    ### YOUR CODE HERE
    input_x = [self.embedding_layer(input_x[:,timestep]) for timestep in range(self.config.num_steps)]
    ### END YOUR CODE
    return input_x
  ...
  def add_model(self, input_x, initial_state):
    ...
    for inx in input_x:
      term1 = torch.matmul(final_state,self.H)
      term2 = torch.matmul(inx,self.I)
      final_state = torch.sigmoid(term1+term2+ self.b1)
      rnn_outputs.append(self.output_drop(final_state))
    ### END YOUR CODE
  ...
  def add_projection(self, rnn_outputs):
    ...
    ### YOUR CODE HERE
    outputs = []
    for hidden_rep in rnn_outputs:
      r = torch.matmul(hidden_rep,self.U) + self.b2
      outputs.append(r)
    ### END YOUR CODE
    return outputs
